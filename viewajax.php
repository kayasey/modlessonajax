...
$validactions = array('viewlessonajax');
$sesskey    = optional_param('sesskey', null, PARAM_RAW);
$handle = fopen('ajaxx.txt','a+');
fwrite($handle, 'viewajax.php');
fwrite($handle, 'POST: '.var_export($_POST, true));
fwrite($handle, 'GET:: '.var_export($_GET, true));
fwrite($handle, 'REQUEST:: '.var_export($_REQUEST, true));
fclose($handle);
...
function viewlessonajax(){
		...
		if ($backtocourse) {
			...
			return array(
					...
					'ajaxredirect'=>new moodle_url('/course/viewajax.php', array('id'=>$course->id, 'action'=>'viewlessonajax','sesskey'=>$sesskey,'action'=>'viewlessonajax')),
					'id'=>$course->id,
					'html' => '',
					'sesskey' => $sesskey,
					'newajaxcall' => true,
		...
		if ($timerestriction = $lesson->get_time_restriction_status()) {  // Deadline restrictions.
			//$outputhtml .= $lessonoutput->header($lesson, $cm, '', false, null, get_string('notavailable'));
			$outputhtml .= $lessonoutput->lesson_inaccessible(get_string($timerestriction->reason, 'lesson', userdate($timerestriction->time)));
			...
			//$outputhtml .= $lessonoutput->header($lesson, $cm, '', false, null, get_string('passwordprotectedlesson', 'lesson', format_string($lesson->name)));
			$outputhtml .= $lessonoutput->login_prompt($lesson, $userpassword !== '');
			//$outputhtml .= $lessonoutput->footer();
			exit();
		} else if ($dependenciesrestriction = $lesson->get_dependencies_restriction_status()) { // Check for dependencies.
			//$outputhtml .= $lessonoutput->header($lesson, $cm, '', false, null, get_string('completethefollowingconditions', 'lesson', format_string($lesson->name)));
			$outputhtml .= $lessonoutput->dependancy_errors($dependenciesrestriction->dependentlesson, $dependenciesrestriction->errors);
			//$outputhtml .= $lessonoutput->footer();
			exit();
		}
		...
		if (empty($pageid)) {
					...
					if (!$DB->count_records('lesson_pages', array('lessonid'=>$lesson->id))) {
						//redirect("$CFG->wwwroot/mod/lesson/edit.php?id=$cm->id"); // no pages - redirect to add pages
							...
							return array(
											'msgerror' => '',
											'info' => 'Mesaj gönderildi',
											'ajaxredirect'=>$CFG->wwwroot.'/mod/lesson/editajax.php',
											'id'=>$cm->id,
											'html' => '',
											'sesskey' => $sesskey,
											'newajaxcall' => true,
			...
			if (($lastpageseen !== false && ($lastpageseen != LESSON_EOL))) {
				...
				if ($lesson->left_during_timed_session($retries)) {
					//$outputhtml .= $lessonoutput->header($lesson, $cm, '', false, null, get_string('leftduringtimedsession', 'lesson'));
					if ($lesson->timelimit) {
						if ($lesson->retake) {
							...
							$outputhtml .= html_writer::div($lessonoutput->message(get_string('leftduringtimed', 'lesson'), $continuelink),
									'center leftduring');
							...
							$outputhtml .= html_writer::div($lessonoutput->message(get_string('leftduringtimednoretake', 'lesson'), $courselink),
									'center leftduring');
						}
					} else {
						$outputhtml .= $lessonoutput->continue_links($lesson, $lastpageseen);
					}
					//$outputhtml .= $lessonoutput->footer();
					exit();
				}
			...
			if ($attemptflag) {
				if (!$lesson->retake) {
					//$outputhtml .= $lessonoutput->header($lesson, $cm, 'view', '', null, get_string("noretake", "lesson"));
					$courselink = new single_button(new moodle_url('/course/view.php', array('id'=>$PAGE->course->id)), get_string('returntocourse', 'lesson'), 'get');
					$outputhtml .= $lessonoutput->message(get_string("noretake", "lesson"), $courselink);
					//$outputhtml .=$lessonoutput->footer();
					exit();
				}
			}
			...
			if (!$pageid = $lessonfirstpageid) {
				//$outputhtml .=$lessonoutput->header($lesson, $cm, 'view', '', null);
		...
		if ($pageid != LESSON_EOL) {
			...
			if (!$canmanage) {
				...
				if (!$lesson->check_time($timer)) {
					...
					return array(
						'msgerror' => '',
						'info' => 'Mesaj gönderildi',
						'ajaxredirect'=>(string)new moodle_url('/mod/lesson/viewajax.php', array('id' => $cm->id, 'pageid' => LESSON_EOL, 'outoftime' => 'normal','sesskey' => $sesskey,'action' => 'viewlessonajax')),
						'id'=>$cm->id,
						'pageid'=>LESSON_EOL,
						'outoftime'=>'normal',
						'html' => '',
			...
			$lessoncontent = str_replace('<div class="row">','',$lessoncontent);
			$lessoncontent = str_replace('<div class="col-3 ','<div class="',$lessoncontent);
			$lessoncontent = str_replace('<div class="col-9 lessoncontents">','',$lessoncontent);
			$lessoncontent = str_replace('<li class="lessonbranchbutton" ','<button class="btn btn-primary lessonbranchbutton" ',$lessoncontent);
			$lessoncontent = str_replace('<ul>','',$lessoncontent);
			$lessoncontent = str_replace('</ul>','',$lessoncontent);
			$lessoncontent = str_replace('</li>','',$lessoncontent);
			$lessoncontent = explode('<div class="box py-3 contents">',$lessoncontent);
			$lessoncontent = implode("\n", array_reverse($lessoncontent));
			$lessoncontent = str_replace('</div></div></div></div>','</div>',$lessoncontent);
			...
			//$outputhtml .= $lessonoutput->header($lesson, $cm, $currenttab, $extraeditbuttons, $lessonpageid, $extrapagetitle);
			if ($attemptflag) {
				// We are using level 3 header because attempt heading is a sub-heading of lesson title (MDL-30911).
				$outputhtml .= $OUTPUT->heading(get_string('attempt', 'lesson', $retries), 3);
			}
			...
			$outputhtml .= $lessoncontent;
			$outputhtml .= $lessonoutput->progress_bar($lesson);
			//$outputhtml .= $lessonoutput->footer();
			...
			lesson_add_fake_blocks($PAGE, $cm, $lesson, $timer);
			//$outputhtml .= $lessonoutput->header($lesson, $cm, $currenttab, $extraeditbuttons, $lessonpageid, get_string("congratulations", "lesson"));
			$outputhtml .= $lessoncontent;
			//$outputhtml .= $lessonoutput->footer();
	}
	//$outputhtml .= '<script src="'.$CFG->wwwroot.'/mod/lesson/ajax.js" type="text/javascript"></script>';
